...
async function gerarMensagem(lead: {
    ...
    const { content } = await anthropic.messages.create({
          ...
          messages: [
                  {
                            role: "user",
                            content: `Voce e um vendedor da Tecnosup, empresa de tecnologia de Cruzeiro/SP que desenvolve sistemas de agendamento para barbearias.
                    ...
                    - Objetivo: gerar resposta com pergunta sobre como gerenciam agendamentos
                    - Tom: humano, direto, sem parecer automatico
                    - Referencia algo especifico do contexto
          ...
          for (const place of (data.results ?? []).slice(0, 5)) {
                  ...
                  if (existing.empty) {
                            ...
                            await docRef.update({
                                        ...
                                        historico: [...lead.historico, { acao: "Mensagem personalizada gerada por IA", data: new Date().toISOString() }],
                                      });
